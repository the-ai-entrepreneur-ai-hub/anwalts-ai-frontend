...
class AnwaltsAIApiClient {
    ...
    async login(email, password) {
        try {
            ...
            const response = await this.post('/auth/login', { email: e, password: p });
            if (response && (response.token || response.token_type === 'bearer')) {
    ...
    async registerFull({ email, name, password, profile = {} }) {
        // Try single-step endpoint first; if missing, fall back to multi-step
        try {
            const response = await this.post('/auth/register-full', { email, name, password, profile });
            if (response && response.token) {
                this.authToken = response.token;
                localStorage.setItem('anwalts_auth_token', response.token);
                if (response.user) localStorage.setItem('anwalts_user', JSON.stringify(response.user));
                return response;
            }
        } catch (err) {
            // Continue to fallback if endpoint not found or fails
            console.warn('register-full not available, falling back to multi-step flow:', err?.message || err);
        }

        // Fallback: /auth/register -> /auth/login -> /api/user/profile
        try {
            // 1) Register basic account
            const reg = await this.post('/auth/register', { email, name, password });
            if (!reg || !reg.id) {
            ...
            if (!loginRes || !(loginRes.token || loginRes.access_token)) {
                throw new Error('Login nach Registrierung fehlgeschlagen');
            }

            // 3) Upsert extended profile
            const snake = {
                first_name: profile.firstName || '',
                last_name: profile.lastName || '',
                firm: profile.firm || undefined,
                role_title: profile.roleTitle || undefined,
                bar_id: profile.barId || undefined,
                jurisdictions: profile.jurisdictions || [],
                practice_areas: profile.practiceAreas || [],
                phone: profile.phone || undefined,
                website: profile.website || undefined,
                marketing_opt_in: !!profile.marketingOptIn,
                consent_at: profile.consentAt || new Date().toISOString(),
                terms_accepted_at: profile.termsAcceptedAt || new Date().toISOString()
            };
            // Remove undefined keys
            Object.keys(snake).forEach(k => snake[k] === undefined && delete snake[k]);

            // Prefer PUT per backend implementation
            await this.put('/api/user/profile', snake);

            // Return unified shape
            return {
                token: this.authToken,
                token_type: 'bearer',
                user: loginRes.user || { id: reg.id, email, name, role: 'assistant' }
            };
        } catch (error) {
    ...
    async generateDocument(title, documentType, templateContent = '', variables = {}, templateId = null) {
        ...
        try {
            ...
            const basePrompt = `Create a ${documentType || 'document'} with title: ${title || 'Untitled'}`;
            const fullPrompt = templateContent
              ? `${basePrompt}\n\nUse the following template content as the structure and language style. Adapt carefully to the instructions.\n\nTEMPLATE:\n${templateContent}\n\nINSTRUCTIONS (if any) may follow from user input.`
              : basePrompt;
    ...
    async legalAnswer(question, userContext = [], preferences = {}) {
        try {
            const payload = {
                question,
                user_context: userContext, // optional short facts from the user/session
                preferences: {
                    language: 'de',
                    max_tokens: 512,
                    temperature: 0.1,
            ...
            const response = await this.post('/ai/legal/answer', payload);
            return response; // expected: { id, answer_md, citations: [...], sources: [...], latency_ms }
        } catch (error) {
            console.error('Legal answer error:', error);
            throw new Error('Rechtsauskunft konnte nicht erzeugt werden');
        }
    }

    async legalFeedback(answerId, feedbackType, comment = '') {
        try {
            const response = await this.post('/ai/legal/feedback', {
                answer_id: answerId,
                feedback_type: feedbackType, // incorrect_citation | unclear_explanation | too_slow | hallucination_detected
                comment
            });
            return response;
        } catch (error) {
            console.error('Legal feedback error:', error);
            throw new Error('Feedback konnte nicht übermittelt werden');
        }
    }

    async legalLearn(correction) {
        try {
            // correction: { question, correct_citation?, correct_text?, links?, tags? }
            const response = await this.post('/ai/legal/learn', correction);
            return response;
        } catch (error) {
            console.error('Legal learn error:', error);
            throw new Error('Lernhinweis konnte nicht gespeichert werden');
        }
    }
    ...
    async getTemplates(category = 'all', type = 'all') {
        try {
            const response = await this.get(`/api/templates?category=${category}&type=${type}`);
            return response;
        } catch (error) {
    ...
    async getTemplate(templateId) {
        try {
            const response = await this.get(`/api/templates/${templateId}`);
            return response;
        } catch (error) {
    ...
    async saveTemplate(name, content, category, type = 'personal') {
        try {
            const response = await this.post('/api/templates', {
                name,
                content,
                category,
                type
    ...
    async updateTemplate(templateId, updates) {
        try {
            const response = await this.put(`/api/templates/${templateId}`, updates);
            return response;
        } catch (error) {
    ...
    async deleteTemplate(templateId) {
        try {
            const response = await this.delete(`/api/templates/${templateId}`);
            return response;
        } catch (error) {
    ...
    async saveClause(category, title, content, tags = [], language = 'de') {
        try {
            const response = await this.post('/api/clauses', {
                category,
                title,
                content,
                tags,
    ...
    async updateClause(clauseId, updates) {
        try {
            const response = await this.put(`/api/clauses/${clauseId}`, updates);
            return response;
        } catch (error) {
    ...
    async deleteClause(clauseId) {
        try {
            const response = await this.delete(`/api/clauses/${clauseId}`);
            return response;
        } catch (error) {
