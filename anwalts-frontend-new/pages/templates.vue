...
const loadTemplates = async () => {
  ...
  try {
    const response = await $fetch<any[]>('/api/templates', {
      headers: resolveAuthHeaders({ accept: 'application/json' }),
      credentials: 'include'
    })
...
const deleteTemplate = async (template: TemplateRecord) => {
  ...
  try {
    ...
    await $fetch(`/api/templates/${template.id}${query}`, {
      method: 'DELETE',
...
.templates-body {
  ...
  padding: 26px clamp(18px, 4vw, 48px) clamp(44px, 6vw, 64px);
}
