<template>
  <PortalShell>
    <!-- Minimal skeleton to avoid any consent flash while checking status -->
    <div v-if="!uiReady" class="email-skeleton" aria-busy="true" aria-live="polite">
    ...
    <div class="email-app">
      ...
      <div class="consent-card">
        ...
        <div class="oauth-buttons">
          ...
          >
            <svg class="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
              <path d="M4 4h16a1 1 0 011 1v14h-6v-5H9v5H4a1 1 0 01-1-1V5a1 1 0 011-1zm7 7h2V9h-2v2z"/>
            </svg>
            <span>Firmen-E-Mail verbinden</span>
          </button>
        </div>

        <div
          v-if="domainConnections.length"
          class="domain-connection-summary"
          role="status"
          aria-live="polite"
        >
          <div class="domain-summary-header">
            <span class="domain-summary-title">Verknüpfte Firmenpostfächer</span>
            <button
              v-if="isAdmin"
              class="domain-summary-manage"
              type="button"
              @click="openDomainWizard"
            >
          ...
          <ul class="domain-summary-list">
            <li v-for="connection in domainConnections" :key="connection.id" class="domain-summary-item">
              <div class="domain-summary-info">
                <span class="domain-summary-email">{{ connection.email }}</span>
                <span class="domain-summary-host">{{ connection.host }}:{{ connection.port }}</span>
              </div>
              <div class="domain-summary-status">
                <span
                  :class="['status-pill', connection.lastStatus === 'connected' ? 'status-pill-good' : 'status-pill-warn']"
                >
                  {{ connection.lastStatus === 'connected' ? 'Verbunden' : connection.lastStatus || 'Unbekannt' }}
                </span>
                <span class="domain-summary-updated">
                  {{ connection.lastVerifiedAt ? formatRelativeDate(connection.lastVerifiedAt) : 'Noch nicht geprüft' }}
                </span>
              </div>
              <button
                v-if="isAdmin"
                class="domain-summary-remove"
                type="button"
                @click="removeDomainConnection(connection.id)"
              >
                Entfernen
              </button>
            </li>
    ...
    <Teleport to="body">
      ...
      <div v-if="showDomainWizard" class="domain-modal-overlay" @click="closeDomainWizard">
        <div class="domain-modal" @click.stop>
          <div class="domain-modal-header">
            <div>
              <h2 class="domain-modal-title">Firmen-E-Mail verbinden</h2>
              <p class="domain-modal-subtitle">Verbinden Sie Postfächer Ihres Kanzlei-Domains via IMAP.</p>
            </div>
            <button class="domain-modal-close" type="button" @click="closeDomainWizard" aria-label="Schließen">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
              </svg>
            </button>
          ...
          <div class="domain-modal-body">
            <div class="domain-form-grid">
              <label class="domain-field">
                ...
                <input v-model="domainWizard.email" type="email" placeholder="partner@kanzlei.de" />
              </label>
              <label class="domain-field">
                <span>IMAP Host</span>
                <input v-model="domainWizard.host" type="text" placeholder="imap.kanzlei.de" />
              </label>
              <label class="domain-field">
                <span>Port</span>
                <input v-model.number="domainWizard.port" type="number" min="1" max="65535" />
              </label>
              <label class="domain-field">
                <span>Benutzername</span>
                <input v-model="domainWizard.username" type="text" placeholder="partner@kanzlei.de" />
              </label>
              <label class="domain-field">
                <span>App-Passwort</span>
                <input v-model="domainWizard.password" type="password" placeholder="••••••••" />
              </label>
              <label class="domain-checkbox">
                <input type="checkbox" v-model="domainWizard.useSSL" />
                <span>Sichere Verbindung (SSL) verwenden</span>
              </label>
            </div>

            <div class="domain-consents">
              <label class="domain-consent-checkbox">
                <input type="checkbox" v-model="domainWizard.mailboxConsent" />
                <span>Ich habe Zugriff auf dieses Postfach und gestatte der Plattform den Abruf der Inhalte.</span>
              </label>
              <label class="domain-consent-checkbox">
                <input type="checkbox" v-model="domainWizard.aiConsent" />
                <span>Ich gestatte der KI, Nachrichten aus diesem Postfach zur Erstellung von Zusammenfassungen zu verarbeiten.</span>
              </label>
            </div>

            <div v-if="domainWizard.error" class="domain-modal-error">{{ domainWizard.error }}</div>
            <div v-if="domainWizard.success" class="domain-modal-success">{{ domainWizard.success }}</div>
          </div>

          <div class="domain-modal-footer">
            <button class="domain-footer-button" type="button" @click="closeDomainWizard">Abbrechen</button>
            <button
              class="domain-footer-button primary"
              type="button"
              :disabled="domainWizard.loading || !canSubmitDomainWizard"
              @click="submitDomainWizard"
            >
              <svg v-if="domainWizard.loading" class="animate-spin w-4 h-4" viewBox="0 0 24 24" fill="none">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/>
              </svg>
              <span>{{ domainWizard.loading ? 'Verbindung wird geprüft…' : 'Postfach verbinden' }}</span>
            </button>
          </div>
        </div>
      </div>
...
const supabaseClient = process.client ? $supabase : null
let authSubscriptionCleanup = null
const { isAdmin } = useAuth()

const domainConnections = ref([])
const showDomainWizard = ref(false)
const domainWizard = reactive({
  email: '',
  host: '',
  port: 993,
  username: '',
...
const canSubmitDomainWizard = computed(() => {
  return Boolean(
    domainWizard.email &&
    domainWizard.host &&
    domainWizard.port &&
    domainWizard.username &&
    domainWizard.password &&
    domainWizard.mailboxConsent &&
    domainWizard.aiConsent
  )
})

const CONNECTION_KEY_PREFIX = 'anwalt.email.connected'
...
const gmailErrorMessages = {
  ...
  account_mismatch: 'Dieses Gmail-Konto gehört zu einem anderen Portal-Benutzer. Bitte melden Sie sich als der richtige Benutzer an und versuchen Sie es erneut.'
}
...
const makeConnectionKey = (userId) => {
  const trimmed = (userId || '').trim()
  if (!trimmed) {
    return null
  }
  return `${CONNECTION_KEY_PREFIX}:${trimmed}`
}

const purgeLegacyConnectionKeys = () => {
  if (typeof window === 'undefined') {
    return
  }
  try {
    localStorage.removeItem(CONNECTION_KEY_PREFIX)
  } catch (err) {
    console.warn('Failed to purge legacy Gmail connection flag', err)
  }
}

const purgeForeignConnectionFlags = (keepUserId = '') => {
  if (typeof window === 'undefined') {
    return
  }
  const prefix = `${CONNECTION_KEY_PREFIX}:`
  const keepKey = makeConnectionKey(keepUserId)
  const keysToRemove = []
  try {
    ...
    console.warn('Failed to enumerate Gmail connection flags', err)
  }
  keysToRemove.forEach((key) => {
    try {
      localStorage.removeItem(key)
    } catch (err) {
      console.warn('Failed to remove Gmail connection flag', key, err)
    }
  })
}

const rememberConnectionState = (active) => {
  if (typeof window === 'undefined') {
    return
  }
  purgeLegacyConnectionKeys()
  const key = makeConnectionKey(currentUserId.value)
  if (!key) {
    return
  }
  try {
    ...
    console.warn('Failed to persist Gmail connection flag', err)
  }
}
...
const handleAuthUserChange = (nextId, { resetView = true } = {}) => {
  const normalized = (nextId || '').trim()
  if (normalized === currentUserId.value) {
    return
  }
  const previousId = currentUserId.value
  if (previousId) {
    const previousKey = makeConnectionKey(previousId)
    if (previousKey && typeof window !== 'undefined') {
      try {
        localStorage.removeItem(previousKey)
      } catch (err) {
        console.warn('Failed to clear previous Gmail connection flag', err)
      }
    }
  }
  purgeForeignConnectionFlags(normalized)
  currentUserId.value = normalized
  stopAutoRefresh()
  rememberConnectionState(false)
  if (resetView) {
    resetEmailState()
    consents.value = { oauth: false, aiReading: false }
    settings.value.aiReadAccess = false
    settings.value.draftOnlyMode = true
    settings.value.consentTimestamp = null
...
const refreshGmailStatus = async ({ eagerSync = true } = {}) => {
  const headers = {}
  ...
  try {
    ...
    if (response.ok) {
      ...
      const wasInbox = currentView.value === 'inbox'
      if (data.connected) {
        connectionSuccess.value = true
        setTimeout(() => { connectionSuccess.value = false }, 2000)
        rememberConnectionState(true)
        gmailConnected.value = true

        consents.value = {
          oauth: Boolean(data.oauth_consent),
          aiReading: Boolean(data.ai_read_consent)
        }

        settings.value.aiReadAccess = Boolean(data.ai_read_consent)
        settings.value.draftOnlyMode = data.draft_only_mode !== undefined
          ? Boolean(data.draft_only_mode)
          : false
        settings.value.consentTimestamp = data.consent_timestamp
          ? new Date(data.consent_timestamp)
          : null

        const desiredLabel = resolveLabelForFolder(activeFolder.value)
        activeLabel.value = desiredLabel
        ...
        if (eagerSync) {
          const manualLoaded = await syncEmails({ manual: true, append: false, labelOverride: desiredLabel })
          if (!manualLoaded && !lastManualSyncReason.value) {
            await syncEmails({ append: false, labelOverride: desiredLabel })
          }
        } else {
          syncEmails({ append: false, labelOverride: desiredLabel }).catch((err) => {
            console.warn('Initial email list load failed', err)
          })

          syncEmails({ manual: true, append: false, labelOverride: desiredLabel })
            .then((manualLoaded) => {
              if (manualLoaded === false && lastManualSyncReason.value && lastManualSyncReason.value !== 'not_linked') {
                console.warn('Initial manual sync did not complete successfully:', lastManualSyncReason.value)
              }
            })
            .catch((err) => {
              console.warn('Background manual sync failed', err)
            })
        }
      } else {
...
const mapDomainConnection = (record) => {
  if (!record) {
    return null
  }
  return {
    ...
    useSSL: record.use_ssl !== false,
    protocol: record.protocol || 'imap',
    mailboxConsent: Boolean(record.mailbox_access_consent),
    aiConsent: Boolean(record.ai_read_consent),
    lastStatus: record.last_status || 'pending',
...
const resetDomainWizard = () => {
  domainWizard.email = ''
  domainWizard.host = ''
  domainWizard.port = 993
  domainWizard.username = ''
  domainWizard.password = ''
  domainWizard.useSSL = true
  domainWizard.mailboxConsent = false
  domainWizard.aiConsent = false
  domainWizard.error = ''
  domainWizard.success = ''
}
...
const openDomainWizard = () => {
  ...
  domainWizard.success = ''
  domainWizard.password = ''
  showDomainWizard.value = true
}

const closeDomainWizard = () => {
  showDomainWizard.value = false
  domainWizard.loading = false
  resetDomainWizard()
}

const fetchDomainConnections = async () => {
  if (!isAdmin.value) {
  ...
  try {
    const response = await fetch('/api/domain-email/connections', {
      headers,
      credentials: 'include'
    })
    ...
    const mapped = Array.isArray(payload.connections)
      ? payload.connections.map(mapDomainConnection).filter(Boolean)
      : []
    domainConnections.value = mapped
    if (mapped.length > 0 && currentView.value !== 'inbox') {
      currentView.value = 'inbox'
    ...
    console.warn('Failed to load domain email connections', err)
    if (!domainConnections.value.length) {
      domainConnections.value = []
    }
  }
}

const submitDomainWizard = async () => {
  if (!canSubmitDomainWizard.value || domainWizard.loading) {
    return
  }
  domainWizard.loading = true
  domainWizard.error = ''
  domainWizard.success = ''
  ...
  const payload = {
    email: domainWizard.email,
    host: domainWizard.host,
    port: domainWizard.port || 993,
    username: domainWizard.username,
    password: domainWizard.password,
    use_ssl: domainWizard.useSSL,
    protocol: 'imap',
    consent_mailbox: domainWizard.mailboxConsent,
    consent_ai: domainWizard.aiConsent
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const detail = data.detail || data.message || 'Verbindung fehlgeschlagen. Bitte prüfen Sie Host und Zugangsdaten.'
      throw new Error(detail)
    }

    const result = await response.json()
    const mapped = mapDomainConnection(result)
    if (mapped) {
      domainConnections.value = [
        mapped,
        ...domainConnections.value.filter(connection => connection.id !== mapped.id)
      ]
    }
    domainWizard.success = 'Postfach verbunden'
    await fetchDomainConnections()
    ...
    console.error('Domain connector error:', err)
    domainWizard.error = err?.message || 'Verbindung fehlgeschlagen. Bitte versuchen Sie es erneut.'
    domainWizard.loading = false
  }
}

const removeDomainConnection = async (connectionId) => {
  if (!isAdmin.value) {
    return
  }
  const confirmed = confirm('Möchten Sie dieses Postfach wirklich entfernen?')
  if (!confirmed) {
    return
  }
  const headers = { Accept: 'application/json' }
  ...
  try {
    const response = await fetch(`/api/domain-email/connections/${connectionId}`, {
      method: 'DELETE',
      headers,
      credentials: 'include'
    })
    if (!response.ok) {
      throw new Error(`Fehler beim Entfernen (${response.status})`)
    }
    domainConnections.value = domainConnections.value.filter(connection => connection.id !== connectionId)
    if (!domainConnections.value.length && !gmailConnected.value) {
      currentView.value = 'consent'
    }
  } catch (err) {
    console.error('Failed to delete domain connection', err)
    alert('Das Postfach konnte nicht entfernt werden. Bitte versuchen Sie es erneut.')
  }
}
...
const sanitizeLegacyProviderParam = () => {
  if (typeof window === 'undefined') {
    return
  }
  try {
    ...
    if (provider === 'outlook') {
      errorMessage.value = 'Die Outlook-Verknüpfung steht nicht mehr zur Verfügung. Bitte nutzen Sie Gmail oder die Firmen-E-Mail-Option.'
      setTimeout(() => { errorMessage.value = '' }, 5000)
    }
  } catch (err) {
    console.warn('Failed to normalise provider parameter', err)
  }
}
...
const resolveCurrentUserId = async () => {
  ...
  if (supabaseClient && supabaseClient.auth && supabaseClient.auth.getSession) {
    try {
      ...
      console.warn('Failed to resolve user id from Supabase session', err)
    }
  }
  ...
  try {
    ...
    console.warn('Failed to read Supabase auth token from storage', err)
  }

  try {
    const portalUser = localStorage.getItem('anwalts_user')
    if (portalUser) {
    ...
    console.warn('Failed to read portal user from storage', err)
  }
...
const scheduleAutoRefresh = () => {
  ...
  autoRefreshHandle = window.setInterval(() => {
    ...
    syncEmails().catch((err) => {
      console.warn('Automatic email refresh failed', err)
    })
  }, 30000)
}
...
watch(domainConnections, (connections) => {
  ...
  } else if (!gmailConnected.value && currentView.value !== 'consent') {
    currentView.value = 'consent'
  }
}, { deep: true })
...
// Lifecycle
onMounted(async () => {
  try { document.cookie = 'active_email_account=; Max-Age=0; path=/'; } catch (_) {}
  sanitizeLegacyProviderParam()
  consumeGmailErrorFromLocation()

  const initialUserId = await resolveCurrentUserId()
  handleAuthUserChange(initialUserId, { resetView: false })
  purgeLegacyConnectionKeys()
  purgeForeignConnectionFlags(currentUserId.value)

  if (supabaseClient && supabaseClient.auth && supabaseClient.auth.onAuthStateChange) {
    try {
      const { data } = supabaseClient.auth.onAuthStateChange((event, session) => {
        const nextId = session?.user?.id || ''
        const shouldReset = event !== 'TOKEN_REFRESHED'
        handleAuthUserChange(nextId, { resetView: shouldReset })
        if (event === 'SIGNED_OUT') {
          rememberConnectionState(false)
        }
        refreshGmailStatus()
          .then(fetchDomainConnections)
          .catch((err) => {
            console.warn('Failed to refresh Gmail status after auth change', err)
          })
      })
      authSubscriptionCleanup = data?.subscription || null
    } catch (err) {
      console.warn('Failed to subscribe to Supabase auth changes', err)
    }
  }

  await Promise.allSettled([
    refreshGmailStatus({ eagerSync: false }),
    fetchDomainConnections()
  ])
  uiReady.value = true
  if (currentView.value === 'inbox' && gmailConnected.value) {
    scheduleAutoRefresh()
  }
})
...
onBeforeUnmount(() => {
  stopAutoRefresh()
  if (authSubscriptionCleanup && typeof authSubscriptionCleanup.unsubscribe === 'function') {
    try {
      authSubscriptionCleanup.unsubscribe()
    } catch (err) {
      console.warn('Failed to unsubscribe Supabase auth listener', err)
    }
  }
...
.domain-summary-item {
  ...
  gap: 1rem;
  padding: 0.5rem 0;
  border-top: 1px solid rgba(91, 124, 230, 0.12);
}

.domain-summary-item:first-child {
  border-top: none;
  padding-top: 0;
}
...
.email-row {
  ...
  padding-right: 5.5rem; /* reserve space for actions without overlap */
  border-bottom: 1px solid var(--border);
  cursor: pointer;
...
.email-footer {
  ...
  justify-content: center;
  gap: 0.5rem;
  padding: 0.75rem 1.5rem 4.25rem; /* enough to clear chat without looking oversized */
  background: var(--surface);
...
.domain-footer-button:hover {
  background: rgba(91, 124, 230, 0.15);
}

.domain-footer-button.primary {
  background: var(--primary);
  color: white;
}

.domain-footer-button.primary:disabled {
  opacity: 0.6;
  cursor: not-allowed;
}

.domain-footer-button.primary:hover:not(:disabled) {
  background: var(--primary-hover);
  box-shadow: 0 8px 20px rgba(91, 124, 230, 0.25);
}
...
/* Email Skeleton */
.email-skeleton { padding: 1rem; }
.skeleton-header { height: 56px; background: linear-gradient(90deg, #f3f4f6 25%, #e5e7eb 37%, #f3f4f6 63%); background-size: 400% 100%; border-radius: 10px; animation: shimmer 1.2s infinite; margin-bottom: 12px; }
.skeleton-body { display: grid; gap: 10px; }
.skeleton-item { height: 48px; border-radius: 10px; background: linear-gradient(90deg, #f3f4f6 25%, #e5e7eb 37%, #f3f4f6 63%); background-size: 400% 100%; animation: shimmer 1.2s infinite; }
