                ...
                <div class="card-body card-body--combined">
                  ...
                  <div class="card-section section-details">
                    ...
                    <div class="card-section-body">
                      ...
                      <div class="field" id="clauseSection">
                        <div class="field-label-row">
...
onMounted(() => {
  ...
  function getAuthHeader() {
    try {
      ...
      if (!token) {
        console.warn('[Documents] Auth token NOT FOUND - user may need to log in')
        return {}
      }
      const bearer = token.startsWith('Bearer ') ? token : `Bearer ${decodeURIComponent(token)}`
      console.log('[Documents] Auth token detected')
      return { Authorization: bearer, 'X-Portal-Auth': bearer }
    } catch (error) {
      console.warn('[Documents] Failed to resolve auth token', error)
      return {}
    }
  }
  ...
  async function persistDocument(options = {}) {
    const {
      ...
      title,
      metadata,
      status: desiredStatus = 'saved',
      silent = false
    } = options || {}

    if (!ep.save) {
      console.warn('[Documents] Save endpoint not configured, skip persistDocument()')
      return null
    }

    const htmlContent = html || preview?.innerHTML || ''
    if (!htmlContent || !htmlContent.trim()) {
      if (!silent) updateFeedbackStatus('Kein Inhalt zum Speichern vorhanden.', 'danger')
      return null
    }

    const resolvedTitle = (title || document.getElementById('docType')?.value || 'Rechtsdokument').trim()
    const resolvedType = document.getElementById('docType')?.value || SELECTED_TEMPLATE?.docType || 'custom'

    try {
      const saved = await backendPostJson(ep.save, {
        title: resolvedTitle,
        html: htmlContent,
        content: htmlContent,
        document_type: resolvedType,
        uploadedFileId: window.__lastUploadId || null,
        metadata: metadata || window.__lastDocMetadata || {},
        status: desiredStatus
      })

      const doc = saved?.document || {}
      const docId = saved?.id || saved?.documentId || doc?.id
      if (docId) window.__lastDocId = docId
      if (doc?.metadata) window.__lastDocMetadata = doc.metadata
      const downloadLinks = saved?.download || doc?.download || null
      if (downloadLinks) window.__lastDownloadLinks = downloadLinks
      exportPdfBtn?.classList.remove('hidden')

      if (!silent) updateFeedbackStatus('Dokument gespeichert. PDF-Download verfügbar.', 'success')
      return saved
    } catch (error) {
      console.error('[Documents] persistDocument failed:', error)
      if (!silent) {
        const detail = error?.message ? String(error.message) : 'Unbekannter Fehler'
        updateFeedbackStatus(`Speichern fehlgeschlagen: ${detail}`, 'danger')
      }
      throw error
    }
  }
  ...
  function showProcessingOverlay(message = 'Dokument wird erstellt...', subtext = 'KI-Analyse läuft') {
    ...
    // Safety valve: never spin indefinitely
    try {
      window.clearTimeout(window.__overlaySafetyTimer)
    } catch(_) {}
    try {
      window.__overlaySafetyTimer = window.setTimeout(() => {
        if (overlay && overlay.getAttribute('data-state') === 'running') {
          console.warn('[Documents] Overlay safety timer: forcing hide after timeout')
          hideProcessingOverlay()
          if (typeof updateFeedbackStatus === 'function') {
            updateFeedbackStatus('Zeitüberschreitung bei der Generierung. Bitte erneut versuchen.', 'danger')
          }
        }
      }, 20000)
    } catch(_) {}
  ...
  function hideProcessingOverlay() {
    ...
    try { window.clearTimeout(window.__overlaySafetyTimer) } catch(_) {}
  }
  ...
  function setClauseSectionVisible(visible) {
    if (!clauseSection) return
    clauseSection.style.display = visible ? '' : 'none'
  }
      ...
      if (Array.isArray(data) && data.length) {
        TEMPLATE_STORE = data.map(t => {
          ...
          return {
            ...
            // Use full plain text for prompts inserted into the input (no truncation)
            prompt: toPlainText(t.content || ''),
            body: t.content || ''
          }
        })
    ...
    return [
      {
        ...
        prompt: 'Parteien, Zweck, Laufzeit, Vertragsstrafe, Gerichtsstand; Bezug: §§ 241, 280 BGB',
        body: '<h2>Geheimhaltungsvereinbarung (NDA)</h2><p>Zwischen [Partei A] und [Partei B] wird folgende Vertraulichkeitsvereinbarung geschlossen. Schutz vertraulicher Informationen, Zweckbindung, Laufzeit, Vertragsstrafe bei Verstößen, Gerichtsstand. Bezug: §§ 241, 280 BGB.</p>'
      },
      {
        id: 'de-mietvertrag-wohnraum',
        title: 'Mietvertrag – Wohnraum',
        docType: 'Mietvertrag (Wohnraum)',
        category: 'Zivilrecht',
        prompt: 'Mietsache, Mietzins, Kaution, Schönheitsreparaturen, Kündigung; Bezug: §§ 535 ff. BGB',
        body: '<h2>Mietvertrag (Wohnraum)</h2><p>Vermieter: [Name], Mieter: [Name]. Mietsache: [Adresse/Objekt]. Mietbeginn, Miethöhe, Nebenkosten, Kaution, Instandhaltung, Kündigungsfristen. Bezug: §§ 535 ff. BGB.</p>'
      },
      {
        id: 'de-arbeitsvertrag',
        title: 'Arbeitsvertrag',
        docType: 'Arbeitsvertrag',
        category: 'Arbeitsrecht',
        prompt: 'Tätigkeit, Arbeitszeit, Vergütung, Urlaub, Probezeit; Bezug: § 611a BGB, NachweisG',
        body: '<h2>Arbeitsvertrag</h2><p>Arbeitgeber [Firma] und Arbeitnehmer [Name] schließen folgenden Arbeitsvertrag. Tätigkeit, Arbeitszeit, Vergütung, Urlaub, Probezeit, Nebentätigkeit, Vertraulichkeit. Bezug: § 611a BGB, Nachweisgesetz.</p>'
      },
      {
        id: 'de-kuendigungsschutzklage',
        title: 'Kündigungsschutzklage',
        docType: 'Klage – Kündigungsschutz',
        category: 'Arbeitsrecht',
        prompt: 'Klageantrag, Kündigungsdatum, Betriebsgröße, Frist § 4 KSchG; Bezug: KSchG, ZPO',
        body: '<h2>Kündigungsschutzklage</h2><p>Kläger [AN] gegen Beklagte [AG]. Antrag: Feststellung, dass die Kündigung vom [Datum] unwirksam ist. Sachverhalt, soziale Gesichtspunkte, Betriebsgröße, Fristwahrung gem. § 4 KSchG. Bezug: KSchG, ZPO.</p>'
      },
      {
        id: 'de-dsgvo-auskunft',
        title: 'Auskunftsersuchen nach Art. 15 DSGVO',
        docType: 'DSGVO – Auskunftsersuchen',
        category: 'Datenschutz',
        prompt: 'Identität, Umfang, Kopien, Frist 1 Monat; Bezug: Art. 12, 15 DSGVO',
        body: '<h2>Auskunftsersuchen</h2><p>Sehr geehrte Damen und Herren, hiermit mache ich mein Recht auf Auskunft nach Art. 15 DSGVO geltend (Verarbeitungszwecke, Kategorien, Empfänger, Speicherdauer) und bitte um Kopie der personenbezogenen Daten. Bezug: Art. 12, 15 DSGVO.</p>'
      },
      {
        id: 'de-uwg-abmahnung',
        title: 'Abmahnung – UWG',
        ...
        prompt: 'Adressat, Wettbewerbsverstoß, Unterlassung, strafbewehrte Unterlassungserklärung, Frist; Bezug: §§ 3, 8, 12 UWG',
        body: '<h2>Abmahnung</h2><p>Sehr geehrte Damen und Herren, wegen des Wettbewerbsverstoßes [Beschreibung] fordern wir die Abgabe einer strafbewehrten Unterlassungserklärung bis [Datum]. Kostenerstattungsvorbehalt. Bezug: §§ 3, 8, 12 UWG.</p>'
      },
      {
        id: 'de-agb-onlineshop',
        title: 'Allgemeine Geschäftsbedingungen (AGB) – Onlineshop',
        docType: 'AGB (Onlineshop)',
        category: 'Verbraucherrecht',
        prompt: 'Geltungsbereich, Vertragsschluss, Preise/Versand, Widerruf, Gewährleistung; Bezug: §§ 305 ff. BGB, § 312g, § 355 BGB',
        body: '<h2>AGB</h2><p>Geltungsbereich, Begriffsbestimmungen, Vertragsschluss, Preise/Versand, Zahlungsarten, Widerruf, Gewährleistung, Haftung, Datenschutz, Schlussbestimmungen. Bezug: §§ 305 ff. BGB, §§ 312g, 355 BGB.</p>'
      },
      {
        id: 'de-widerrufsbelehrung',
        title: 'Widerrufsbelehrung – Verbraucher',
        docType: 'Widerrufsbelehrung',
        category: 'Verbraucherrecht',
        prompt: 'Frist, Ausübung, Folgen, Muster-Widerrufsformular; Bezug: § 312g, § 355 BGB',
        body: '<h2>Widerrufsbelehrung</h2><p>Sie haben das Recht, binnen vierzehn Tagen ohne Angabe von Gründen diesen Vertrag zu widerrufen. Fristbeginn, Ausübung des Widerrufs, Folgen des Widerrufs. Bezug: § 312g, § 355 BGB.</p>'
      },
      {
        id: 'de-gmbh-gesellschaftsvertrag',
        title: 'Gesellschaftsvertrag – GmbH',
        docType: 'Gesellschaftsvertrag (GmbH)',
        category: 'Gesellschaftsrecht',
        prompt: 'Firma, Sitz, Gegenstand, Stammkapital, Geschäftsführung, Vertretung; Bezug: GmbHG',
        body: '<h2>Gesellschaftsvertrag GmbH</h2><p>Firma und Sitz, Unternehmensgegenstand, Stammkapital, Geschäftsanteile, Geschäftsführung, Vertretung, Gesellschafterversammlung, Gewinnverwendung. Bezug: GmbHG.</p>'
      },
      {
        id: 'de-widerspruch-mahnbescheid',
        title: 'Widerspruch gegen Mahnbescheid',
        docType: 'Widerspruch – Mahnbescheid',
        category: 'Zivilprozess',
        prompt: 'Aktenzeichen, Frist, Umfang (voll/teilweise), Begründung optional; Bezug: §§ 688 ff. ZPO',
        body: '<h2>Widerspruch gegen Mahnbescheid</h2><p>Gegen den Mahnbescheid vom [Datum], Az. [xx], lege ich Widerspruch ein (voll/teilweise). Begründung erfolgt gesondert. Bezug: §§ 688 ff. ZPO.</p>'
      }
    ]
  }
  ...
  async function loadClauses(){
    ...
    try {
      ...
      if (Array.isArray(data) && data.length) {
        ...
        setClauseSectionVisible(true)
      } else {
        CLAUSE_STORE = []
        setClauseSectionVisible(false)
      }
      if (CLAUSE_STORE.length) renderClauseChips(CLAUSE_STORE)
    } catch (err) {
      CLAUSE_STORE = []
      setClauseSectionVisible(false)
    }
...
const ensureAuthenticatedMessaging = () => {
  ...
  // Hide clause section until authenticated/loaded
  setClauseSectionVisible(false)
  setInlineMessage(templateStatus, 'Beispielvorlagen – bitte anmelden, um eigene Vorlagen zu sehen.', 'info')
}
...
const triggerInitialLoads = () => {
  ...
  function renderInlineTemplates(list){
    ...
    if (!items.length) {
      ...
      // Helpful CTAs when no items are rendered
      const ctas = document.createElement('div')
      ctas.style.marginTop = '8px'
      ...
      openBtn.type = 'button'
      openBtn.className = 'inline-template-view'
      openBtn.textContent = 'Alle Vorlagen öffnen'
      openBtn.addEventListener('click', (e) => {
        e.preventDefault()
        navigateToTemplates(e)
      })
      ...
      try {
        if (!portalUser.value) {
          ...
          loginBtn.href = '/login?redirect=' + encodeURIComponent('/documents')
          loginBtn.className = 'inline-template-apply'
          loginBtn.textContent = 'Jetzt anmelden'
          ctas.appendChild(loginBtn)
        }
      } catch (_) {}

      cont.appendChild(ctas)
      return
    }
    items.forEach(t => {
      ...
      const loginLink = (!portalUser || !portalUser.value)
        ? ('<a class="inline-template-view" href="/login?redirect=' + encodeURIComponent('/documents') + '">Anmelden</a>')
        : ''
      el.innerHTML = `
  ...
  function applyTemplate(tpl){
    ...
    if (req) {
      ...
      // Focus requirements so users see the insertion immediately
      try { req.focus({ preventScroll: false }); req.scrollIntoView({ behavior: 'smooth', block: 'center' }) } catch(_) {}
    }
  ...
  async function generate(){
    ...
    try {
      ...
      if (resolvedDocId || window.__lastDownloadLinks) {
        exportPdfBtn?.classList.remove('hidden')
      }
      ...
      try {
        ...
        console.warn('[Documents] Auto-save after generation failed:', persistError)
      }
  ...
  const feedbackGroupEl = actionBarEl?.querySelector('.feedback-group') || null
  let __feedbackSelection = null
  let __feedbackSaving = false

  function setFeedbackSelection(nextState) {
    __feedbackSelection = nextState || null
  ...
  function setFeedbackBusy(isBusy) {
    if (!feedbackGroupEl) return
    const buttons = feedbackGroupEl.querySelectorAll('.feedback-button')
    feedbackGroupEl.classList.toggle('is-busy', Boolean(isBusy))
    buttons.forEach(btn => {
      if (isBusy) {
    ...
    if (typeof setHint === 'function' && isBusy) {
      try { setHint('', '0') } catch (_) {}
    }
  }

  function resolveStatusEndpoint() {
    const statusPath = ep.status || '/api/documents/status'
    return statusPath
  }

  async function submitDocumentFeedback(targetStatus, options = {}) {
    if (__feedbackSaving) return
    const docId = window.__lastDocId
    if (!docId) {
      updateFeedbackStatus('Bitte zuerst ein Dokument generieren.', 'danger')
      return
    }
    const previousState = __feedbackSelection
    const { successMessage, tone = 'info' } = options || {}
    try {
      __feedbackSaving = true
      setFeedbackBusy(true)
      updateFeedbackStatus('Feedback wird gespeichert...', 'info')
      const endpoint = resolveStatusEndpoint()
      await backendPostJson(endpoint, { doc_id: docId, status: targetStatus })
      setFeedbackSelection(targetStatus)
      if (successMessage) {
        updateFeedbackStatus(successMessage, tone)
      } else {
        updateFeedbackStatus('Feedback übernommen.', 'info')
      }
    } catch (error) {
      console.error('[Documents] ❌ Feedback update failed:', error)
      setFeedbackSelection(previousState)
      const detail = error?.message ? String(error.message) : 'Unbekannter Fehler'
      updateFeedbackStatus(`Feedback konnte nicht gespeichert werden: ${detail}`, 'danger')
    } finally {
      __feedbackSaving = false
  ...
  acceptBtn?.addEventListener('click', () => {
    ...
    if (!content) {
      updateFeedbackStatus('Kein Dokument vorhanden. Bitte zuerst generieren.', 'danger')
      return
    }
    submitDocumentFeedback('accepted', {
      successMessage: 'Dokument akzeptiert und zur Übergabe markiert.',
      tone: 'success'
    })
  })
  ...
  rejectBtn?.addEventListener('click', () => {
    ...
    submitDocumentFeedback('rejected', {
      successMessage: 'Dokument zur Überarbeitung markiert.',
      tone: 'danger'
    })
  })
  ...
  saveBtn?.addEventListener('click', async () => {
    ...
    try {
      await persistDocument({
        ...
        metadata: window.__lastDocMetadata || {},
        status: 'saved',
        silent: false
      })
    } catch (_) {
      // Feedback already handled within persistDocument
    }
  })
  ...
  exportPdfBtn?.addEventListener('click', async () => {
    ...
    try {
      const linkFromState = window.__lastDownloadLinks?.pdf
      let docId = window.__lastDocId
      if (!docId && window.__lastDownloadLinks?.docx) {
        const match = /\/api\/documents\/([^/]+)\//.exec(window.__lastDownloadLinks.docx)
        if (match && match[1]) docId = match[1]
      }

      if (!docId && !linkFromState) {
        const autoSave = await persistDocument({
          html: preview.innerHTML,
          title: document.getElementById('docType')?.value || 'Rechtsdokument',
          metadata: window.__lastDocMetadata || {},
          status: 'saved',
        ...
        const doc = autoSave?.document || {}
        docId = autoSave?.id || autoSave?.documentId || doc?.id || null
        if (autoSave?.download) window.__lastDownloadLinks = autoSave.download
      }

      if (!docId && !window.__lastDownloadLinks?.pdf) {
        updateFeedbackStatus('Konnte keine Dokument-ID bestimmen. Bitte erneut speichern.', 'danger')
        return
      }

      const url = linkFromState || window.__lastDownloadLinks?.pdf || `${ep.exportBase || '/api/documents'}/${docId}/export?format=pdf`
      const resp = await backendFetchRaw(url)
      const blob = await resp.blob()
      const a = document.createElement('a')
      a.href = URL.createObjectURL(blob)
      a.download = `${slugifyFilename(document.getElementById('docType')?.value || 'Dokument')}.pdf`
  ...
  try {
    ...
    // Guard dev-only overlay: log in production without noisy UI artifacts.
    console.error('[Documents] onMounted failed:', error)
    console.error('[Documents] Error stack:', error.stack)
    if (typeof updateFeedbackStatus === 'function') {
      updateFeedbackStatus('Fehler beim Laden. Bitte Seite neu laden.', 'danger')
    }
    // Do not render fixed overlays in production; avoid throwing to keep shell usable.
  }
})
...
/* removed legacy template-card styles (unused) */
...
.generate-spinner {
  ...
  /* Animation only when overlay is visible */
}

/* Only animate spinner when overlay is NOT hidden */
.generate-overlay:not(.hidden) .generate-spinner {
