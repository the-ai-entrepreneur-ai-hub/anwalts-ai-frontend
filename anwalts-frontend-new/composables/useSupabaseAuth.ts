...
let browserClientSingleton: BrowserSupabaseClient | null = null
let serverClientSingleton: ServerSupabaseClient | null = null
...
export const useSupabaseAuth = () => {
  ...
  const supabase = (() => {
    if (process.client) {
      const globalScope = globalThis as typeof globalThis & { __ANWALTS_SUPABASE__?: BrowserSupabaseClient }
      if (!browserClientSingleton) {
        if (globalScope.__ANWALTS_SUPABASE__) {
          browserClientSingleton = globalScope.__ANWALTS_SUPABASE__
        } else {
          ...
          browserClientSingleton = createBrowserClient(config.public.supabaseUrl, config.public.supabaseKey)
          globalScope.__ANWALTS_SUPABASE__ = browserClientSingleton
          // Log the client type for debugging
          console.log('[OAuth] Browser client type:', browserClientSingleton.constructor.name)
          console.log('[OAuth] Browser client storage key:', (browserClientSingleton as any).storageKey || 'unknown')
        }
      }
      return browserClientSingleton!
    } else {
      if (!serverClientSingleton) {
        serverClientSingleton = createClient(config.public.supabaseUrl, config.public.supabaseKey, {
          auth: {
  ...
  const fetchSessionFromServer = async () => {
    try {
      const fetcher = process.server
        ? requestFetch
        : (nuxtApp?.$fetch ?? (globalThis as any).$fetch ?? null)

      if (typeof fetcher !== 'function') {
        console.warn('[OAuth] No fetch implementation available for session sync')
        return null
      }

      const response = await (fetcher as any)('/api/auth/session', {
        credentials: 'include'
      }) as {
      ...
      if (response?.session?.access_token) {
        if (process.client) {
          try {
            ...
            localStorage.setItem('anwalts_auth_token', response.session.access_token)
            if (response.user?.id) {
            ...
            console.warn('Failed to persist JWT session tokens', storageError)
          }
        }
        return null
      }
