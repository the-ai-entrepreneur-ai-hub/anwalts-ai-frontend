...
export default defineNuxtPlugin(() => {
  ...
  const initTemplateCreate = () => {
    // Fix invalid selector by quoting attribute value and add a fallback to title attribute
    let btn: HTMLElement | null = null;
    try {
      btn = qs('button[aria-label*="Neue Vorlage"],button[title*="Neue Vorlage"]')
    } catch (e) {
      // Selector failed (older engines); silently ignore
      btn = null
    }
    if (!btn) return;
