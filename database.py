...
class UserInDB:
    def __init__(self, id, email, name, role, password_hash, created_at, is_active=True):
        self.id = id
        self.email = email
        self.name = name
        self.role = role
        ...
        # Backward-compat: some schemas may omit this column
    ...
    async def update_template(self, template_id: uuid.UUID, user_id: uuid.UUID, title: str = None, content: str = None, category: str = None):
        """Update a template"""
        try:
            async with self.get_connection() as conn:
                updates = []
                ...
                if updates:
                    ...
                    query = f"UPDATE templates SET {', '.join(updates)} WHERE id = ${param_count} AND user_id = ${param_count + 1}"
    ...
    # Clause methods
    async def get_clauses(self, user_id: uuid.UUID, category: str = None, language: str = None):
    ...
    async def create_clause(self, user_id: uuid.UUID, title: str, content: str, category: str = None, language: str = None):
        """Create a new clause"""
        try:
            async with self.get_connection() as conn:
                ...
                return {"id": str(clause_id), "title": title, "content": content, "category": category, "language": language}
        except Exception as e:
    ...
    async def create_clipboard_entry(self, user_id: uuid.UUID, content: str, entry_type: str = "text"):
    ...
    # Analytics methods
    async def create_analytics_event(self, user_id: uuid.UUID, event_type: str, data: dict = None):
    ...
    # Document methods
    async def create_document(self, user_id: uuid.UUID, title: str, content: str, document_type: str = None):
    ...
    async def create_api_token(self, user_id: uuid.UUID, token: str, expires_at: datetime, last4: str):
    ...
    async def get_api_token_owner(self, token: str):
        ...
        try:
            async with self.get_connection() as conn:
                ...
                if row:
                    return UserInDB(**dict(row), password_hash="", created_at=datetime.utcnow())
                return None
        except Exception as e:
    ...
    async def create_call_request(self, name: str, phone: str, email: str = None, message: str = None):
        ...
        try:
            async with self.get_connection() as conn:
                ...
                return {"id": str(request_id), "name": name, "phone": phone, "email": email, "message": message}
        except Exception as e:
